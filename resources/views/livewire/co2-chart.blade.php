<div class="flex relative w-1/2 h-full border-2 border-blue-600 rounded-lg">
    ...
    <div class="flex w-1/2 h-full justify-center items-center gap-4 p-4">
        <div class="w-full flex justify-center">
            <div id="chartCO2_1" class="chart"></div>
        </div>
        <div class="w-full flex justify-center">
            <div id="chartCO2_2" class="chart"></div>
        </div>
    </div>
    <div class="flex flex-col w-1/2 lg:flex-row justify-center items-center gap-4 p-4">
        <div class="w-full flex justify-center">
            <div id="lineChartCO2" class="chart"></div>
        </div>
    </div>
</div>
...
@push('scripts')
    ...
    <script>
        ...
        var root = am5.Root.new("chartCO2_1");
        ...
        // Add clock hand
        var handDataItemCO2_1 = axis.makeDataItem({
            value: 0
        });

        var hand = handDataItemCO2_1.set("bullet", am5xy.AxisBullet.new(root, {
        ...
        axis.createAxisRange(handDataItemCO2_1);

        handDataItemCO2_1.get("grid").set("visible", false);
        handDataItemCO2_1.get("tick").set("visible", false);

        setInterval(() => {
            handDataItemCO2_1.animate({
                key: "value",
                to: {{ $co2Records->where('co2_type', 'CO2 1')->last()->co2_value ?? '0' }},
                duration: 1500,
        ...
        chart.children.unshift(am5.Label.new(root, {
            html: "<div style=\"text-align: center;\"><p style=\"font-size: 14px;\"><b>{{ $co2Records->where('co2_type', 'CO2 1')->last()->co2_value ?? 'No Data' }}</b></p></div>",
            x: am5.percent(50),
            centerX: am5.percent(50),
            paddingTop: 70,
            paddingBottom: 0
    ...
    <script>
        // Create root and chart
        var root = am5.Root.new("chartCO2_2");
        ...
        // Add clock hand
        var handDataItemCO2_2 = axis.makeDataItem({
            value: 0
        });

        var hand = handDataItemCO2_2.set("bullet", am5xy.AxisBullet.new(root, {
            sprite: am5radar.ClockHand.new(root, {
                radius: am5.percent(99),
                innerRadius: am5.percent(0),
                topWidth: 0,
        ...
        axis.createAxisRange(handDataItemCO2_2);

        handDataItemCO2_2.get("grid").set("visible", false);
        handDataItemCO2_2.get("tick").set("visible", false);

        setInterval(() => {
            handDataItemCO2_2.animate({
                key: "value",
                to: {{ $co2Records->where('co2_type', 'CO2 2')->last()->co2_value ?? '0' }},
                duration: 1500,
                easing: am5.ease.out(am5.ease.cubic)
            });
        }, 2000);
        ...
        // Add an HTML-enabled title
        chart.children.unshift(am5.Label.new(root, {
            html: "<div style=\"text-align: center;\"><p style=\"font-size: 14px;\"><b>{{ $co2Records->where('co2_type', 'CO2 2')->last()->co2_value ?? 'No Data' }}</b></p></div>",
            x: am5.percent(50),
            centerX: am5.percent(50),
            paddingTop: 70,
            paddingBottom: 0
    ...
    <script>
        ...
        var root = am5.Root.new("lineChartCO2");
