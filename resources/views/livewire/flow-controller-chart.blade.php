<div class="flex relative w-1/2 h-full border-2 border-blue-600 rounded-lg">
    ...
    <div class="flex w-1/2 h-full justify-center items-center gap-4 p-4">
        <div class="w-full flex justify-center">
            <div id="chartFC1" class="chart"></div>
        </div>
        <div class="w-full flex justify-center">
            <div id="chartFC2" class="chart"></div>
        </div>
    </div>
    <div class="flex flex-col w-1/2 lg:flex-row justify-center items-center gap-4 p-4">
        <div class="w-full flex justify-center">
            <div id="lineChartFC" class="chart"></div>
        </div>
    </div>
</div>
@push('scripts')
    ...
    <script>
        ...
        var root = am5.Root.new("chartFC1");
        ...
        // Add clock hand
        var handDataItemFC1 = axis.makeDataItem({
            value: 0
        });

        var hand = handDataItemFC1.set("bullet", am5xy.AxisBullet.new(root, {
        ...
        axis.createAxisRange(handDataItemFC1);

        handDataItemFC1.get("grid").set("visible", false);
        handDataItemFC1.get("tick").set("visible", false);

        setInterval(() => {
            handDataItemFC1.animate({
                key: "value",
                to: {{ $flowControllerRecords->where('flow_controller_type', 'FC 1')->last()->flow_controller_value ?? '0' }},
                duration: 1500,
        ...
        chart.children.unshift(am5.Label.new(root, {
            html: "<div style=\"text-align: center;\"><p style=\"font-size: 14px;\"><b>{{ $flowControllerRecords->where('flow_controller_type', 'FC 1')->last()->flow_controller_value ?? 'No data' }}</b></p></div>",
            x: am5.percent(50),
            centerX: am5.percent(50),
            paddingTop: 70,
            paddingBottom: 0
    ...
    <script>
        // Create root and chart
        var root = am5.Root.new("chartFC2");
        ...
        // Add clock hand
        var handDataItemFC2 = axis.makeDataItem({
            value: 0
        });

        var hand = handDataItemFC2.set("bullet", am5xy.AxisBullet.new(root, {
            sprite: am5radar.ClockHand.new(root, {
                radius: am5.percent(99),
                innerRadius: am5.percent(0),
                topWidth: 0,
        ...
        axis.createAxisRange(handDataItemFC2);

        handDataItemFC2.get("grid").set("visible", false);
        handDataItemFC2.get("tick").set("visible", false);

        setInterval(() => {
            handDataItemFC2.animate({
                key: "value",
                to: {{ $flowControllerRecords->where('flow_controller_type', 'FC 2')->last()->flow_controller_value ?? '0' }},
                duration: 1500,
                easing: am5.ease.out(am5.ease.cubic)
            });
        }, 2000);
        ...
        // Add an HTML-enabled title
        chart.children.unshift(am5.Label.new(root, {
            html: "<div style=\"text-align: center;\"><p style=\"font-size: 14px;\"><b>{{ $flowControllerRecords->where('flow_controller_type', 'FC 2')->last()->flow_controller_value ?? 'No data' }}</b></p></div>",
            x: am5.percent(50),
            centerX: am5.percent(50),
            paddingTop: 70,
            paddingBottom: 0
    ...
    <script>
        ...
        var root = am5.Root.new("lineChartFC");
